/**
 * INLINED COPY of topoloom v0.3.0's layout module (src/layout/index.ts),
 * with internal imports rewritten to the published 0.2.7 subpath exports.
 *
 * Why: this example pins @khalidsaidi/topoloom 0.2.7 (latest on npm today).
 * 0.2.7's planarizationLayout has no `{ mode: 'orthogonal' }` option and its
 * orthogonalLayout rejects vertices of degree > 4, so the v0.3.0 module is
 * carried along verbatim. Once 0.3.0 is on npm, delete this file and use:
 *
...
import type { Graph, EdgeId, VertexId } from '@khalidsaidi/topoloom/graph';
import { buildHalfEdgeMesh, selectOuterFace } from '@khalidsaidi/topoloom/embedding';
import type { HalfEdgeMesh, FaceId } from '@khalidsaidi/topoloom/embedding';
import { buildDual, routeEdgeFixedEmbedding } from '@khalidsaidi/topoloom/dual';
